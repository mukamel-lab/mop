...
from .decomposition_helpers import prep_pca
from .statistics import batch_mean_and_std
...
def cluster_cross_validation(loom_file,
    ...
    Returns:
        loss (float): the average euclidian distance of a validtation cell to the
            centroid of its predicted cluster.
        
    """
    
    if verbose:
        clust_log.info('Cross validating cluster params for {}'.format(loom_file))
        t0 = time.time()
    with loompy.connect(loom_file) as ds:
    ...
    for key, values in splits.items():
        with loompy.connect(loom_file) as ds:
            ...
            ds.ca[train_label.format(key)] = [i in valid_idx [[i not in values\
                                for i in valid_idx]] for i in np.arange(num_cells)]
    loss = 0
    for split in splits.keys():
        ...
        for label in np.unique(pred_labels):
            ...
            loss += np.sum(sklearn.metrics.pairwise_distances(mean_pcs, cluster_vals))
